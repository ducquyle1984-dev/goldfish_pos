...
# Firebase CLI does not have a projects:delete command.
# We use the Cloud Resource Manager API with the token cached by firebase login.
...
if ($accessToken) {
    ...
    if ($deleted) {
        Write-Host " ✓ Firebase project '$ProjectId' deleted." -ForegroundColor Green
    } else {
...
if ($RemoveFromRegistry) {
    ...
    Write-Host "[2/2] Skipping registry update (use -RemoveFromRegistry to also remove from client-projects.json)." -ForegroundColor Gray
}
